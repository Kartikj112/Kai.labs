  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (let s = 0; s < strandCount; s++) {
        ...
        // Depth cue: strands alternate near/far, softened with blur + lower
        // alpha the "farther" they sit, so the helix reads as occupying
        // real space rather than a flat, uniform pattern.
        const depth = 0.55 + 0.45 * Math.cos((s / strandCount) * Math.PI * 2)
        ctx!.filter = depth < 0.75 ? `blur(${((0.75 - depth) * 3).toFixed(2)}px)` : 'none'

        for (let y = 0; y < H; y += 2) {
          const x1 = xB + Math.sin(y * freq + t * spd + ph) * amp
          const x2 = xB + Math.sin(y * freq + t * spd + ph + Math.PI) * amp
          const al  = (0.055 + 0.03 * Math.abs(Math.sin(y * freq + t * spd))) * depth
