...
export function WgsSummaryNode({ state, onRestart, onHub }: Props) {
  ...
  const buildMarkdown = () => {
    ...
    const L: string[] = [];
    L.push('# Bacterial WGS — Your Workflow');
    L.push(`_Generated by the KAI Decision Engine · ${date}_`);
    L.push('');
    L.push('## Configuration');
    tags.forEach(t => L.push(`- **${t.k}:** ${t.v}`));
    L.push('');
    L.push('## Recommended workflow');
    wf.forEach((s, i) => { L.push(`${i + 1}. **${s.l}**`); L.push(`   - ${s.d}`); });
    L.push('');
    L.push('## Publication checklist');
    cl.forEach(c => L.push(`- [ ] ${c}`));
    L.push('');
    L.push('---');
  ...
  const handleDownload = () => {
    downloadText('kai-wgs-workflow.md', buildMarkdown());
    setDownloaded(true);
